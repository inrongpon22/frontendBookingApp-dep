...
export default function EditServiceTime(props: IParams) {
    ...
    const [daysOpen, setDaysOpen] = useState<string[]>(
        props.serviceTime[editValue].daysOpen == undefined
            ? []
            : props.serviceTime[editValue].daysOpen
    );
    const [selectedSlots, setSelectedSlots] = useState<number[]>([]);
    const [duration, setDuration] = useState(props.duration);
    const [openTime, setOpenTime] = useState(props.openTime.substring(0, 5));
    const [closeTime, setCloseTime] = useState(props.closeTime.substring(0, 5));
    const [guestNumber, setGuestNumber] = useState(1);
    const [isManually, setIsManually] = useState(editValue ? true : false);
    const [manualCapacity, setManualCapacity] = useState<IBookingSlot[]>(props.serviceTime[editValue].slotsTime);
    ...
    const generateTimeSlots = (
        ...
        while (currentTime <= endTime) {
            ...
            const endTimeString = `${endTimeHours.toString().padStart(2, "0")}:${endTimeMinutes
                .toString()
                .padStart(2, "0")}`;
    ...
    const handleSubmit = () => {
        const insertData = {
            ...
            slotsTime: timeSlots.filter((_, index) => selectedSlots.includes(index)),
        };
    ...
    useEffect(() => {
        if (props.serviceTime[editValue].slotsTime !== undefined) {
            const selectedSlots: number[] = [];
            props.serviceTime[editValue].slotsTime.forEach((slot) => {
                const index = timeSlots.findIndex(
                    (timeSlot) => timeSlot.startTime === slot.startTime && timeSlot.endTime === slot.endTime
                );
                if (index !== -1) {
                    ...
                    {isManually == true ? (
                            ...
                            {selectedSlots
                                ...
                                .map((element) => (
                                    <div key={element}>
                                        <div className="flex justify-between">
                                            <div className="p-3">
                                                {timeSlots[element].startTime} - {timeSlots[element].endTime}
                                            </div>
                                            <div className="flex justify-between gap-3 items-center p-3">
                                                ...
                                                {manualCapacity.find(
                                                    (item) =>
                                                        item.startTime == timeSlots[element].startTime &&
                                                        item.endTime == timeSlots[element].endTime
                        ...
                        >
                            <div className="p-3">
                                ...
                                <button onClick={() => setIsManually(true)} disabled={(timeSlots.length == 0 || timeSlots[0].startTime == '') || selectedSlots.length == 0}>
                                    <u
                                        style={{
                                            color: "#020873",
                                            fontSize: "12px",
