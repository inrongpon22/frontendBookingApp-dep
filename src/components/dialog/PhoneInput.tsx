  ...
  return (
    <section>
      <div className="flex-auto">
        <p className="text-[1.5rem] font-semibold ">{query.get("accessCode") ? t("form:business:auth:login") :t("verifyPhoneNumber")}</p>
        <span className="text-[1rem]">{t("verifyPhoneNumberDesc")}</span>
      </div>
      <input
        type="text"
        className={`w-full p-4 rounded-lg my-5 ${
      ...
      />
      {formik.touched.phoneNumbers && formik.errors.phoneNumbers && (
        <span className="text-[1rem] text-rose-500">
      ...
      <button
        type="button"
        disabled={isLoading}
        className="w-full items-center justify-center bg-[#35398F] text-white p-4 rounded-lg font-bold"
      ...
      <span className="text-[1rem]">{t("or")}</span>
      </div>
      <button
